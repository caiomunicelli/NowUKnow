...
class CategoriaRepository {
    ...
    async createCategoria(categoria) {
        ...
        const newCategoria = new Categoria(result.insertId, categoria.nome, categoria.descricao, categoria.imagem, new Date());
        return newCategoria;
    }
    ...
    async getAllCategorias() {
        ...
        return rows.map(row => new Categoria(row.id, row.nome, row.descricao, row.imagem, row.data_criacao));
    }

    // Buscar categoria por ID
    async getCategoriaById(id) {
        const connection = await this.dbConnection.connect();
        const [rows] = await connection.execute("SELECT * FROM Categorias WHERE id = ?", [id]);

        if (rows.length === 0) return null;
        const row = rows[0];
        return new Categoria(row.id, row.nome, row.descricao, row.imagem, row.data_criacao);
    }
    ...
    async updateCategoria(categoria) {
        ...
        return result.affectedRows > 0 ? "Categoria atualizada com sucesso." : "Nenhuma categoria alterada.";
    }
