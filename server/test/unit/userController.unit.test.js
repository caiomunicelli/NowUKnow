...
describe("UserController - Unit Tests", () => {
  ...
  it("Cria novo usuário", async () => {
    const mockRequest = {
      ...
      query: sinon
        ...
        .resolves({
          recordset: [
            { id: 1, nome: "Lorenzo Calca", email: "lorenzo@example.com" },
          ],
        }),
    };
    dbStub.resolves({ request: () => mockRequest });

    const result = await usuarioController.criarUsuario(
      "Lorenzo Calca",
      "lorenzo@example.com",
      "new@Password123",
      "estudante"
    ...
    assert.deepStrictEqual(result, {
      sucesso: true,
      usuario: { id: 1, nome: "Lorenzo Calca", email: "lorenzo@example.com" },
  ...
  it("Recupera um usuário pelo ID", async () => {
    const mockRequest = {
      ...
      query: sinon
        .stub()
        .resolves({ recordset: [{ id: 1, nome: "Caio Calca" }] }),
    };
    dbStub.resolves({ request: () => mockRequest });
    ...
    assert.deepStrictEqual(result, {
      sucesso: true,
      usuario: { id: 1, nome: "Caio Calca" },
    }); //Verifica se o usuário foi recuperado
  });
  ...
  it("Atualiza um usuário existente", async () => {
    ...
    sinon
      .stub(UserRepository.prototype, "getUserById")
      .resolves({ id: 1, nome: "LG Souza" });
    dbStub.resolves({ request: () => mockRequest });

    const result = await usuarioController.atualizarUsuario(
      1,
      "Arthur Doe",
      "arthur@example.com",
      "new@Password123",
