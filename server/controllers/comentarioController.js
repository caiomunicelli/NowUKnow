...
class ComentarioController {
    ...
    validarDados(comentario) {
        ...
        if (!comentario.texto || comentario.texto.length === 0) {
            errors.push({ campo: 'texto', mensagem: "O texto do comentário é obrigatório." });
        }
    ...
    async criarComentario(postagemId, usuarioId, texto, parentId) {
        const comentario = new Comentario(0, postagemId, usuarioId, texto, parentId);
        ...
        try {
            const novoComentario = await comentarioService.createComentario(comentario);
    ...
    async listarComentarioPorId(id) {
        try {
            ...
            if (!comentario) {
                return { sucesso: false, erros: [{ campo: 'id', mensagem: "Comentário não encontrado." }] };
            }
    ...
    async atualizarComentario(id, texto) {
        ...
        const comentarioAtualizado = new Comentario(id, comentarioExistente.postagemId, comentarioExistente.usuarioId, texto || comentarioExistente.texto, comentarioExistente.parentId);
        ...
        try {
            const resultadoAtualizacao = await comentarioService.updateComentario(comentarioAtualizado);
    ...
    async listarComentariosPorPostagem(postagemId) {
        try {
            const comentarios = await comentarioService.getComentariosPorPostagem(postagemId);
            return { sucesso: true, comentarios };
        } catch (error) {
            throw new Error('Erro ao buscar comentários da postagem: ' + error.message);
        }
    }
}
