...
router.post('/', verifyJWT, async (req, res) => {
    ...
    try {
        const resultado = await certificacaoController.criarCertificacao(nome, descricao, requisitos, imagem, categoriaId, nivel);
        if (!resultado.sucesso) {
            return res.status(400).json({ errors: resultado.erros });
        }
        res.status(201).json(resultado.certificacao);
    } catch (error) {
        res.status(500).json({ error: 'Erro ao criar certificação', details: error.message });
    }
});
...
router.get('/', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Erro ao listar certificações', details: error.message });
    }
});
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Erro ao buscar certificação', details: error.message });
    }
});
...
router.put('/:id', verifyJWT, async (req, res) => {
    ...
    const { nome, descricao, requisitos, imagem, categoriaId, nivel } = req.body;
    try {
        const resultado = await certificacaoController.atualizarCertificacao(id, nome, descricao, requisitos, imagem, categoriaId, nivel);
        if (!resultado.sucesso) {
            return res.status(400).json({ errors: resultado.erros });
        }
        res.status(200).json({ mensagem: resultado.mensagem });
    } catch (error) {
        res.status(500).json({ error: 'Erro ao atualizar certificação', details: error.message });
    }
});
...
router.delete('/:id', verifyJWT, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Erro ao deletar certificação', details: error.message });
    }
});
