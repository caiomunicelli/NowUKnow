...
router.post('/', verifyJWT, async (req, res) => {
    ...
    try {
        const resultado = await categoriaController.criarCategoria(nome, descricao, imagem);
...
router.put('/:id', verifyJWT, async (req, res) => {
    ...
    const { nome, descricao, imagem } = req.body;
    try {
        const resultado = await categoriaController.atualizarCategoria(id, nome, descricao, imagem);
        if (!resultado.sucesso) {
            return res.status(400).json({ errors: resultado.erros });
        }
