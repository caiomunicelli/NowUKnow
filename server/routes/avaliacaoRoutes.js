...
router.post('/', verifyJWT, async (req, res) => {
    ...
    try {
        const resultado = await avaliacaoController.criarAvaliacao(usuarioId, postagemId, nota, comentario);
...
router.put('/:id', verifyJWT, async (req, res) => {
    ...
    try {
        const resultado = await avaliacaoController.atualizarAvaliacao(id, nota, comentario);
        if (!resultado.sucesso) {
            return res.status(400).json({ erros: resultado.erros });
        }
...
router.get('/postagem/:postagemId', async (req, res) => {
    ...
    try {
        const resultado = await avaliacaoController.listarAvaliacoesPorPostagem(postagemId);
        res.status(200).json(resultado.avaliacoes); // Retorna as avaliações da postagem
    } catch (error) {
        res.status(500).json({ error: 'Erro ao buscar avaliações da postagem', details: error.message });
    }
});
